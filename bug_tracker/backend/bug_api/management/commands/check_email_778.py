...
class Command(BaseCommand):
    help = 'Check emails for Bug 778 and analyze priority extraction'

    def handle(self, *args, **options):
        self.stdout.write('Checking emails for Bug 778...')
        self.check_email()
    ...
    def check_email(self):
        ...
        self.stdout.write("Searching for emails with Bug ID 778...")
        status, messages = mail.search(None, 'SUBJECT', 'BUG-778')
        email_ids = messages[0].split()
        self.stdout.write(self.style.SUCCESS(f'Found {len(email_ids)} emails for Bug 778'))
